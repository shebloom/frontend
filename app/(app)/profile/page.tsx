                    ...
                    <div key={appt.id} className="flex gap-3 p-3 rounded-2xl border border-bloom-50 bg-slate-50">
                      ...
                      <div className="flex items-center gap-2 mt-1.5 flex-wrap">
                        ...
                        {(() => {
                          ...
                          if (isJoinable) {
                            const isDoctor = (profile?.role as string) === 'doctor';
                            const callStarted = appt.call_started;
                            return (
                              <div className="w-full mt-1.5 p-2 bg-amber-50 border border-amber-200 rounded-xl text-[10px] text-amber-800 font-semibold flex flex-col gap-1.5">
                                <div className="flex items-center gap-1.5">
                                  ...
                                  <span>
                                    {isDoctor
                                      ? 'You can start this consultation call now.'
                                      : callStarted
                                      ? 'Dr. Deepa has initiated the call. You can join now!'
                                      : 'Waiting for Dr. Deepa to start the consultation...'}
                                  </span>
                                </div>
                                <button
                                  onClick={() => router.push(`/home?joinCall=${appt.id}`)}
                                  className={cn(
                                    "w-full py-1.5 font-bold text-xs rounded-lg transition cursor-pointer flex items-center justify-center gap-1",
